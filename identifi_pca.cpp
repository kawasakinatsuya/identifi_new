...
public:
  ...
  : Node("Identification_node"),tf_buffer_(this->get_clock()),tf_listener_(tf_buffer_)
  {
    ...
    this->declare_parameter("planarity_min", 0.4);
...
private:
  void cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    for(auto& kv : cell_info){
      ...
      if(cell_pca(*ds_map, c, n, planarity, scattering)){
        c.normal = n;
  ...
  bool cell_pca(const pcl::PointCloud<pcl::PointXYZ>& cloud, const Cell& c, Eigen::Vector3f& cell_normal, double& cell_planarity, double& cell_scattering)
  {
    if((int)c.pc_indices.size() < this->get_parameter("pca_min_points").as_int()){
      return false;
    }
    Eigen::Vector4f centroid;
    pcl::compute3DCentroid(cloud, c.pc_indices, centroid);

    Eigen::Matrix3f covariance_matrix;
    pcl::computeCovarianceMatrixNormalized(cloud, c.pc_indices, centroid, covariance_matrix);
    covariance_matrix = 0.5f * (covariance_matrix + covariance_matrix.transpose());

    Eigen::SelfAdjointEigenSolver<Eigen::Matrix3f> es(covariance_matrix);
    auto eigenvalues = es.eigenvalues();
    auto eigenvectors = es.eigenvectors();

    double val3 = (double)eigenvalues(0);
    double val2 = (double)eigenvalues(1);
    double val1 = (double)eigenvalues(2);
    cell_normal = evec.col(0).cast<float>();
    cell_normal.normalize();

    cell_planarity = (val2 - val3) / val1;
    cell_scattering = val3 / val1;

    if(std::isfinite(cell_planarity) && std::isfinite(cell_scattering)){
      return true;
    }
  }
      ...
      for(auto& indx : c.pc_indices){
        ...
        if(c.unknown || !std::isfinite(c.ground_z)){
          ...
          if(h_diff > h_obs){
            ...
            if(c.pca_on && (c.planarity >= planatiry_min) && c.scattering <= scat_max){
              //乗り越え可能な障害物
              //紫色
              r = 255;
              g = 0;
